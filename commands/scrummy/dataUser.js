...
const slashCommandExecute = async (interaction) => {
  ...
  try {
    ...
    const imageBuffer = chartBuilder.makeUserHoursChart(timeCardInRange[0].discordName, new Date(start), new Date(end), timeCardInRange)
    const attachment = new AttachmentBuilder(imageBuffer, { name: 'userData.png' })
    const embed = new EmbedBuilder()
      .setTitle(`Chart of Time Worked for ${timeCardInRange[0].discordName}`)
      .setImage('attachment://userData.png')
